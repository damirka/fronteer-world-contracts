...
public fun withdraw(
    ...
    let inventory = e.module_mut<Inventory>(request, internal::permit()).inner_mut();
    let (requirement, frame) = request.take_next(internal::permit<Withdrawal>());
...
public fun deposit_template(
    ...
    assert!(req.is<Deposit>());
    assert!(args.length() <= 1);

    // NOTE: if you're building a complex PTB by combining different behaviors in
    //       an action, then you know what you're doing.
    //       if not, we tell frontend what the requirement is.
    let item = if (args.length() == 1) {
        args.pop_back()
    } else {
        // TODO: unpack Deposit data
        tx::item(option::none(), option::none())
    };
